...
En este proyecto, el **rendimiento académico** se operacionaliza mediante las calificaciones registradas por asignatura. En el contexto del Sistema Educativo Plurinacional de Bolivia se emplea una escala de 1 a 100 puntos y el documento institucional consultado fija 51 puntos como referencia de aprobación (Ministerio de Educación del Estado Plurinacional de Bolivia, 2021).
...
Los **Sistemas de Alerta Temprana** son herramientas analíticas destinadas a priorizar estudiantes que requieren revisión a partir de indicadores históricos o conductuales (Macfadyen & Dawson, 2010). Una alerta no equivale a un diagnóstico: debe acompañarse de revisión humana, contexto pedagógico y un protocolo de intervención.

## 2.4 Consideraciones Éticas y Privacidad

El tratamiento de datos de estudiantes menores exige minimización, seudonimización, control de acceso y una finalidad pedagógica definida. Los identificadores personales no deben utilizarse como variables predictoras ni publicarse en repositorios abiertos. Asimismo, las alertas no deben producir sanciones o decisiones automáticas; su función es señalar casos para revisión docente. El presente proyecto adopta estas restricciones como condiciones de uso del prototipo.

---

## 2.5 Fundamentos de Machine Learning en Clasificación Supervisada
...
### 2.5.1 Árboles de Decisión (Decision Tree Classifier)
Un Árbol de Decisión es un modelo no paramétrico que particiona recursivamente el espacio de características mediante reglas lógicas de decisión jerárquicas (James et al., 2013). En cada nodo interno $m$, el algoritmo evalúa la mejor división $j$ de una característica $x_j$ y un umbral $t$ que maximiza la reducción de la impureza de Gini ($I_G$):

$$I_G(m) = 1 - \sum_{k=0}^{1} p_{mk}^2$$

donde $p_{mk}$ es la proporción de instancias pertenecientes a la clase $k$ en el nodo $m$. En este proyecto, el Árbol de Decisión se parametriza con `max_depth=3` y `min_samples_leaf=10` para limitar su complejidad; esta decisión no elimina por sí sola el riesgo de sobreajuste.

### 2.5.2 Random Forest Classifier y Balanceo de Clases
...
### 2.5.3 Redes Neuronales Artificiales: Perceptrón Multicapa (MLP)
...
En el proyecto se implementa una capa oculta de cuatro neuronas (`hidden_layer_sizes=(4,)`), regularización L2 (`alpha=0.1`) y tasa de aprendizaje inicial de 0,0001, utilizando la función de activación Unidad Rectificada Lineal (ReLU):
...
## 2.6 Tratamiento del Desbalance de Clases en Datos Educativos

Cuando la proporción de reprobación es muy baja, la exactitud global puede ser engañosa porque un clasificador que favorece la clase mayoritaria obtiene muchos aciertos sin detectar casos positivos. El proyecto adopta tres medidas: separación temporal entre entrenamiento y prueba, ponderación de clases en Árbol y Random Forest, y reporte de *recall*, precisión, F1, *balanced accuracy* y precisión promedio. Estas medidas permiten una evaluación más informativa, pero no compensan la existencia de solo seis transiciones positivas.

---

## 2.7 Sistemas Híbridos de Soporte a la Decisión (Expert Rules + ML Inference)
...
Esta regla evita clasificar operativamente como bajo riesgo un registro completo que ya contiene calificaciones reprobatorias. Su puntaje se muestra separado de la probabilidad del modelo y no debe interpretarse como una probabilidad calibrada ni como garantía de detección futura.

---

## 2.8 Stack Tecnológico del Proyecto

El desarrollo técnico del proyecto utiliza bibliotecas de código abierto sobre **Python 3.12** (Pedregosa et al., 2011; Streamlit, 2024):
